...
public class FLLRobotGameParser implements Parser {
    ...
    private String requestLogin(CookieManager cookieManager, String url, String targetUrl, String user, String password) {
        return executeRequest(cookieManager, url, b -> b.header("Content-Type", "application/x-www-form-urlencoded"),
    ...
    private HttpClient createHttpClient(CookieManager cookieManager) {
        return HttpClient.newBuilder().followRedirects(HttpClient.Redirect.ALWAYS)
    ...
    private Team parseTeam(Element e) {
        try {//FIXME ccsQ = "span"
            var name = Objects.requireNonNull(e.selectFirst("span"), "Name Element could not be found!").ownText();
            var teamId = Objects.requireNonNull(e.selectFirst(".team-id"), "Team ID could not be found!").text();

            var scores = e.select("td").stream().map(Element::text)
            ...
            throw new IllegalArgumentException("Unparsable Element!" + System.lineSeparator() + e, ex);
        }
    }
    ...
    private void updatePairings(Document doc, Competition competition) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            pairings.forEach(e -> {
                ...
                if (a.size() == 1) {
                    //FREE SLOT DETECTED
                    System.out.println(a.getFirst().text() + " ||||| " + "FREIER SLOT (HAT KEINE PKTE)");
    ...
    private void checkMatch(String path, Competition competition) {
        String match = requestPageAfterLogin(cookieManagers.get(competition.getName()), environment + "/" + path);
